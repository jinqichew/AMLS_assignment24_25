...
def cnn_B():
    ...
    # preprocess the data to be used in PyTorch model
    X_train = torch.from_numpy(train_images).float().permute(0, 3, 1, 2) / 255.0
    y_train = torch.from_numpy(train_labels).long().squeeze()
    X_val = torch.from_numpy(val_images).float().permute(0, 3, 1, 2) / 255.0
    y_val = torch.from_numpy(val_labels).long().squeeze()
    X_test = torch.from_numpy(test_images).float().permute(0, 3, 1, 2) / 255.0
    ...
    # define model
    class CNN_taskB(nn.Module):
        def __init__(self):
            super(CNN_taskB, self).__init__()
            ...
            self.fc = nn.Linear(32 * 7 * 7, 8)  # 8 output classes
    ...
    print(f"Task B CNN Test Accuracy: {test_accuracy:.4f}")
